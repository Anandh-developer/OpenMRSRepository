...
test.describe.serial('Test suite for OpenMRS E2E', () => {
    test.beforeAll("Test setup", async ({ browser }) => {
        ...
        helperFunctions = new HelperFunctions();// helperFunctions have all the date related calculated related
        ...
        testData = excelFileReader.getTestData("OM_TC01_DS002");//Return the tetst data in the map format by the defined data set id
    })

    test("Test user should be successfully navigate to the openmrs application URL", async ({ }) => {
    ...
    test("Verify user can able to enter user name and password", async ({ }) => {
    ...
    test("Verify user can able to choose any of the session and click on login button", async ({ }) => {
    ...
    test("Verify user successfully logged in by valid user name and password", async ({ }) => {
    ...
    test("Verify user can able to click on rigister patient button", async ({ }) => {
    ...
    test("Veirfy user can able to enter the patient's Demographics details and contact information ", async ({ }) => {
    ...
    test("Verify patient name and address, contact informations are populated correctly in confirm screen", async ({ }) => {
    ...
    test("Verify patiend details page is redirected to electronic medical record screen and age is calculated correctly", async ({ }) => {
    ...
    test("Veirfy when user click on start visit button application should ask start visit popup and user can able to confirm it", async () => {
    ...
    test("Admin can able to upload the attachment in visit screen attachment section", async ({ }) => {
    ...
    test("Veirfy admin can able to redirect to patient details screen", async ({ }) => {
    ...
    test("Verify attachement got added to the patient details screen", async ({ }) => {
    ...
    test("Veirfy admin can able to do the End visit process", async () => {
    ...
    test("Veirfy user can able to enter the vitals in while do the visit", async ({ }) => {
        ...
        await waitUtils.waitForLocator(visitorScreenLocator.getCaptureVitalsButtons());        
    ...
    test("Capture Height and weight and check the BMI is calculated correctly", async ({ }) => {
    ...
    test("Verify admin can able to save the vitals details and redirected to patiend details screen record", async () => {
    ...
    test("Veirfy Admin can able to see the right height and weight and BMI in patient details screen", async ({ }) => {
    ...
    test("Verify recent visit have one more entry and tags should be updated with vitals", async ({ }) => {
...
await waitUtils.waitForLocator(visitorScreenLocator.getClicnicFacingMenuAttachementDateValidator());
    ...
    test("Verify admin can able to merge the recent two visits", async ({ }) => {
    ...
    test("verify after merging the visit the no of visit should be one and tags are properly merged with their names", async ({ }) => {
await waitUtils.waitForLocator(visitorScreenLocator.getClicnicFacingAttachementTextValidator());
    ...
    test("Verify user can not able to enter future date in past visit section", async ({ }) => {
    ...
    test("Verify deleted user can not be found in list view", async ({ }) => {
    ...
    test("Verify deleted patient record is not visible in search grid with", async ({ }) => {
        ...
        await waitUtils.waitForLocator(visitorScreenLocator.getNoPatientFoundVerification());//wait for No patient found message
    ...
    test.afterEach("Screenshot Setup ", async ({ }, testInfo) => {
        ...
        test.afterAll("Tear down set up ",async({})=>{
